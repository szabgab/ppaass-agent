...
impl TryFrom<Socks5Address> for SocketAddr {
    ...
    fn try_from(socks5_addr: Socks5Address) -> Result<Self, Self::Error> {
        match socks5_addr {
            Socks5Address::IpV4(ip, port) => Ok(SocketAddr::V4(SocketAddrV4::new(
            ...
            Socks5Address::Domain(host, port) => {
                ...
                let result = addresses.get(0).ok_or(AgentError::Other(format!(
                    "Fail to parse address: {address_string}"
                )))?;
                Ok(*result)
            }
        }
...
impl From<Socks5Address> for UnifiedNetAddress {
    fn from(value: Socks5Address) -> Self {
        match value {
            Socks5Address::IpV4(host, port) => {
                UnifiedNetAddress::Ip(SocketAddr::new(IpAddr::V4(host.into()), port))
            }
            Socks5Address::IpV6(host, port) => {
                let mut ip_cursor = Cursor::new(host);
                UnifiedNetAddress::Ip(SocketAddr::new(
            ...
            Socks5Address::Domain(host, port) => UnifiedNetAddress::Domain { host, port },
        }
    }
}

impl TryFrom<UnifiedNetAddress> for Socks5Address {
    type Error = AgentError;
    fn try_from(net_addr: UnifiedNetAddress) -> Result<Self, Self::Error> {
        match net_addr {
            UnifiedNetAddress::Ip(socket_address) => match socket_address {
                SocketAddr::V4(ip_v4_address) => Ok(Socks5Address::IpV4(
                    ip_v4_address.ip().octets(),
                    ip_v4_address.port(),
                )),
                SocketAddr::V6(ip_v6_address) => Ok(Socks5Address::IpV6(
                    ip_v6_address.ip().octets(),
                    ip_v6_address.port(),
                )),
            },
            UnifiedNetAddress::Domain { host, port } => Ok(Socks5Address::Domain(host, port)),
        }
    }
}
