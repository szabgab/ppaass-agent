    ...
    pub(crate) fn parse(input: &mut impl Buf) -> Result<Socks5Address, AgentError> {
        if !input.has_remaining() {
            return Err(AgentError::Other(format!(
                "Input bytes exhausted, remaining: 0"
            )));
        }
...
impl From<Socks5Address> for NetAddress {
    fn from(value: Socks5Address) -> Self {
        match value {
            Socks5Address::IpV4(ip, port) => {
                NetAddress::Ip(SocketAddr::V4(SocketAddrV4::new(ip.into(), port)))
            }
            Socks5Address::IpV6(ip, port) => {
                NetAddress::Ip(SocketAddr::V6(SocketAddrV6::new(ip.into(), port, 0, 0)))
            }
            Socks5Address::Domain(host, port) => NetAddress::Domain { host, port },
        }
    }
}

impl TryFrom<NetAddress> for Socks5Address {
    type Error = AgentError;
    fn try_from(net_addr: NetAddress) -> Result<Self, Self::Error> {
        match net_addr {
            NetAddress::Ip(SocketAddr::V4(addr)) => {
                Ok(Socks5Address::IpV4(addr.ip().octets(), addr.port()))
            }
            NetAddress::Ip(SocketAddr::V6(addr)) => {
                Ok(Socks5Address::IpV6(addr.ip().octets(), addr.port()))
            }
            NetAddress::Domain { host, port } => Ok(Socks5Address::Domain(host, port)),
        }
    }
}
